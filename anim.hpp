...
{
public:
	 vector<IntRect> frames, frames_flip;// кадры
	float currentFrame, speed; //текущий кадр
	bool loop;// loop показвает зациклена ли анимация,
	bool flip;//отобр зеркально
	bool isPlaying; // проигрывать аним  
	Sprite sprite;
	...
	void tick(float time)
	{
		...
		int i = currentFrame;
		sprite.setTextureRect( frames[i] );//текущий кадр
   ...
   //загр.спрайтов
	void create(String name, Texture &texture, int x, int y, int w, int h, int count, float speed, int step=0, bool Loop=true)
	{
		...
		{			
			...
			a.frames_flip.push_back( IntRect(x+i*step+w, y, -w, h)  );// отображение
		}
		animList[name] = a;
		currentAnim = name;
	}

	//ф-я для  загрузки спрайтов из файла XML
	...
	float getH()  {return animList[currentAnim].frames[0].height;}//высота

	float getW() {return animList[currentAnim].frames[0].width;}//ширина
