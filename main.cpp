...
class Entity //osnova abstract class
{
public:
	float x,y,dx,dy,w,h;
...
public:
	...
	void update(float time)
	{
		...
		if (Health<=0) {anim.set("explode");dx=0; //if collision - BOOM!!!
...
public:
	...
	bool onLadder;//lestnica
	bool shoot, hit;
	 map< string,bool> key;
	 set<Object> Counter;
	 Text score;

	PLAYER(AnimationManager &a, Level &lev,int x,int y, Text& Score):Entity(a,x,y)
	{	
	...
	void KeyCheck()
	{
		...
		if (key["Up"])
		{
			...
			if (STATE==stay || STATE==walk) { dy=-0.27f; 
	...
	void Collision(int num)//столкновения
	{	
		
		for (int i=0;i<obj.size();i++) 
			if (getRect().intersects(obj[i].rect))
			{
				if((obj[i].name == "collectable")&&(num==1)){
					char b[10];
					Counter.insert(obj[i]);//сбор цветов
					score.setString(string("Score: ") + string(itoa(Counter.size(),b,10)));
				}
				if (obj[i].name=="solid")//твердая поверхность
				{           
				...
				if (obj[i].name=="ladder")	{ onLadder=true; }//карабкаться

				if (obj[i].name=="SlopeLeft")//наклонная поверхность
				...
				if (obj[i].name=="SlopeRight")//накл вправо
				{   FloatRect r = obj[i].rect;
...
{  
	///////////// инициализация ///////////////////////////
	RenderWindow window(VideoMode(800, 600), "Lucky Boy");
	Object f, s;
	
	View view( FloatRect(0, 0, 800, 600) );

	Font font;
	font.loadFromFile("font1.otf");
	Text SCORE("Score: 0",font);
	...
	PLAYER Mario(anim, lvl, pl.rect.left, pl.rect.top, SCORE);

	PlayerScores plScores;

	Music music;
	music.openFromFile("files/zimmer.ogg");
	music.play();
	...
	{ 
		...
		window.setView(view);
		Mario.score.setPosition(view.getCenter());
		Mario.score.move(-view.getSize().x/2 + 30, -view.getSize().y/2);
